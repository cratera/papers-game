...
// NOTE: I hate having the CSS here... should import it.
const container = css`
  display: flex;
...
function StepWelcome () {
  return (
   <>
     <span css={logo} role="img" aria-label="logo">🎲</span>
     <h1 css={ theme.typography.h1 }>Welcome!</h1>
     <p css={ [paragraph, theme.typography.secondary] }>
...
function StepName({ defaultValue, onChange }) {
  return (
    <label css={step}>
      <span>How should we call you?</span>
      <input type="text" autoFocus defaultValue={defaultValue} css={input} onChange={ onChange }></input>
    </label>
  )
}
...
function StepAvatar({ onChange, avatar }) {
  return (
    <label css={step}>
      ...
      />
      {avatar
        ? <img css={avatarImg} src={avatar} alt="Your avatar" />
        : <div css={ avatarPlace}><span role="img" aria-hidden="true">📸</span> Upload a picture</div>
      }
    </label>
  )
}

function StepReady({ name, avatar }) {
  return (
    <div css={step}>
      <p>You're ready!</p>
      <p css={ theme.typography.h1}>{name}</p>
      <div css={memeContainer} aria-hidden="true">
        <img css={memeFace} src={avatar} alt=""></img>
        <svg css={memeBody} version="1.0" xmlns="http://www.w3.org/2000/svg" width="458.667" height="518.667" viewBox="0 0 344 389">
...
export default function Home(props) {
  ...
  const stepMap = {
    ...
    2: () => <StepAvatar
                ...
                avatar={state.avatar} />,
    3: () => <StepReady name={state.name} avatar={state.avatar} />
  }
  ...
  function updateProfile(key, event) {
    return {
      ...
      avatar: async () => {
        ...
        const compressedFile = await compressImg(file, { maxWidthOrHeight: 600 });
  ...
  return (
    <div css={container}>
      { state.step > 0 &&
        <Button css={ btnBack } variant="flat" onClick={goBackStep}>Go back</Button>
      }
      <div css={body}>
        { stepMap[state.step]() }
      </div>
      { state.step < 3
        ? <Button hasBlock onClick={goNextStep}>Next</Button>
        : <>
          <Button hasBlock>Create a Room</Button>
          <Button hasBlock variant="light">Join a Room</Button>
        </>
      }
