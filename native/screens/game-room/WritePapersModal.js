...
import { ScrollView, View, TextInput, Text, TouchableHighlight } from 'react-native';
...
export default function WritePapersModal({ isOpen, onClose }) {
  ...
  // const setPapersWordsThrottled = React.useRef(throttle(q => Papers.setWords(q), 3000)).current;
  // const handleScrollDebounced = React.useRef(debounce(handleScroll, 250)).current;
  ...
  return (
    <Modal visible={isOpen} onClose={onClose}>
      <Text style={[Theme.typography.small, Theme.typography.secondary, Styles.header]}>
        {/* prettier-ignore */}
        {/* <svg style={Styles.headerSvg} viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
  ...
  function focusTextareaAtSlide(index) {
  ...
  function renderSliders() {
    ...
    for (let i = 0; i < wordsGoal; i++) {
      ...
      papers.push(
        <View style={[Styles.slide, StylesIsActive]} data-slide={i + 1} key={i}>
          <TextInput
            onChangeText={text => handleWordChange(text, i)}
            placeholder="Write here..."
            {...(isActive ? { autoFocus: true } : {})}
            style={[
              { height: 32, marginVertical: 4, color: Theme.colors.grayDark },
              Styles.textarea,
            ]}
          ></TextInput>
        </View>
